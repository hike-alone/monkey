...
void ctl_hold_position_now(void)
{
#if BUILD_LEVEL == 5
    ctrl_gt iq_feedback = mtr_ctrl.idq0.dat[phase_q];
    ctl_set_mech_mode_rt(&mech_ctrl, MECH_MODE_POSITION, iq_feedback);
    if (mech_ctrl.pos_if != NULL)
    {
        ctl_set_mech_target_position(&mech_ctrl, mech_ctrl.pos_if->revolutions, mech_ctrl.pos_if->position);
    }
    else
    {
        ctl_set_mech_target_position(&mech_ctrl, 0, float2ctrl(0.0f));
    }
#endif // BUILD_LEVEL == 5
}
void ctl_set_position_target_rev_pu(int32_t revolutions, parameter_gt pos_pu)
{
#if BUILD_LEVEL == 5
    if (pos_pu >= 1.0f)
        pos_pu = 0.999f;
        ...
        pos_pu = 0.0f;
    ctl_set_mech_target_position(&mech_ctrl, revolutions, float2ctrl(pos_pu));
#else
    GMP_UNUSED_VAR(revolutions);
    GMP_UNUSED_VAR(pos_pu);
#endif // BUILD_LEVEL == 5
}
...
void ctl_init()
{
    ...
    mtr_ctrl_init.kid = 0.1 * CONTROLLER_FREQUENCY;
    mtr_ctrl_init.kiq = 0.1 * CONTROLLER_FREQUENCY;
    mtr_ctrl_init.kpd = 1.2;
    mtr_ctrl_init.kpq = 1.2;
    ...
    mech_init.pos_kp = 0.3f;
    mech_init.pos_ki = 0.1f;

    mech_init.vel_kp = 5.0f;
    mech_init.vel_ki = 0.002f * CONTROLLER_FREQUENCY;
...
// attach motor current controller with input port
    ...
    ctl_set_foc_core_vdq_ref(&mtr_ctrl, 0.1, 0.1);
    ...
    ctl_enable_foc_core_current_ctrl(&mtr_ctrl);
    ctl_set_foc_core_idq_ref(&mtr_ctrl, float2ctrl(0.0), float2ctrl(0.1));

#elif BUILD_LEVEL == 4
    // Basic Speed close loop
    ctl_enable_foc_core_current_ctrl(&mtr_ctrl);
    ctl_set_mech_ctrl_mode(&mech_ctrl, MECH_MODE_VELOCITY);
    ctl_set_mech_target_velocity(&mech_ctrl, 0.3);

#elif BUILD_LEVEL == 5
    // Position servo control (position loop)
    ctl_enable_foc_core_current_ctrl(&mtr_ctrl);
    ctl_hold_position_now();
    // Optional: tune position loop gains if needed.
    // Position loop usually uses very small KI, or pure P.
...
#if BUILD_LEVEL == 5
                // ADC calibration also clears mech controller state.
                // Re-arm hold mode to avoid unintended motion at enable.
                ctl_hold_position_now();
#endif // BUILD_LEVEL == 5

 ...
 */
GMP_STATIC_INLINE ctrl_gt ctl_calc_scurve_pos_err_4param(int32_t t_revs, ctrl_gt t_ang, int32_t f_revs, ctrl_gt f_ang)
{
...
GMP_STATIC_INLINE void ctl_step_scurve_planner_tdm(ctl_scurve_planner_t* planner)
{
    ...
    if (cur_slot == 1)
    {
        ...
        ctrl_gt dist_to_go = ctl_calc_scurve_pos_err_4param(planner->target_revs, planner->target_angle,
                                                            planner->planner_revs, planner->planner_angle);
    ...
    {
        ctrl_gt following_error =
            ctl_calc_scurve_pos_err_4param(planner->planner_revs, planner->planner_angle, planner->pos_if->revolutions,
                                           planner->pos_if->position);
